...
def update_lambda_code(lambda_client, config, region):
    """
    Method to update the current lambda code on AWS
    """
    fm_prefix = config["fm_prefix"]
    project_name = config["project_name"]
    func_name = config["lambda_function_name"]
    # The code bucket where the lambda code is present
...
def create_lambda(config, region=None):
    ...
    # check if the lambda func exists and if it does then ask the user for updation
    if lambda_function_exists(lambda_client, func_name):
        update = str(
            input(
                f"A lambda function by the name {func_name} exists. "
