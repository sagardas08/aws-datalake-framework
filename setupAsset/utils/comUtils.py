...
def insert_asset_item_dynamoDB(asset_json_file, asset_id, region):
  ...
  print('Inserting {} info in {}.data_asset table in {}'.format(asset_id, global_config["fm_prefix"], region))
...
def insert_asset_cols_dynamoDB(asset_col_json_file, asset_id, region):
  ...
  print('Inserting column info in {}.data_asset.{} table in {}'.format(global_config["fm_prefix"], asset_id, region))
  for rows in asset_col_config["columns"]:
    item = json.dumps(rows)
    jsonItem=json.loads(item)
    asset_col_table = dynamodb.Table('{}.data_asset.{}'.format(global_config["fm_prefix"], asset_id))
...
def create_asset_detail_table(asset_id, region):
  ...
  print('Creating the table {}.data_asset.{} in {}'.format(global_config["fm_prefix"], str(asset_id), region))
...
def create_src_s3_dir_str(asset_id, asset_json_file, region):
  ...
  os.system('aws s3api put-object --bucket "{}" --key "{}/init/dummy"'.format(bucket_name,asset_id))
  os.system('aws s3api put-object --bucket "{}" --key "{}/error/dummy"'.format(bucket_name,asset_id))
  os.system('aws s3api put-object --bucket "{}" --key "{}/masked/dummy"'.format(bucket_name,asset_id))
  os.system('aws s3api put-object --bucket "{}" --key "{}/error/dummy"'.format(bucket_name,asset_id))
  os.system('aws s3api put-object --bucket "{}" --key "{}/logs/dummy"'.format(bucket_name,asset_id))
...
def set_bucket_event_notification(asset_id, asset_json_file, region):
  ...
  sns_name = global_config["fm_prefix"] + '-' + str(src_sys_id) + '-init-file-creation' 
  sns_arn = 'arn:aws:sns:' + region + ':' + global_config['aws_account'] + ':' + sns_name
