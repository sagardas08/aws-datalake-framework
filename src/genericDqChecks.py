...
args = getResolvedOptions(
    sys.argv, ["source_path", "source_id", "asset_id", "exec_id"]
)
...
if asset.validate_schema(source_df):
    try:
        ...
        check = Check(
            spark, CheckLevel.Warning, "Deequ Data Quality Checks"
        )
        ...
        result.repartition(1).write.csv(
            result_s3_path, header=True, mode="overwrite"
        )
    ...
    stop()
    raise Exception(
        "Halting the execution due to schema irregularities in the dataset"
    )
